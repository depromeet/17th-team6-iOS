...
private extension MyProfileView {
    ...
    private var tabHeaderSection: some View {
        HStack {
            tabButton(title: "인증", index: MyProfileFeature.State.Tab.feed.rawValue)
            tabButton(title: "기록", index: MyProfileFeature.State.Tab.session.rawValue)
        }
        .padding(.top, 16)
        .padding(.horizontal, 20)
        .overlay(
    ...
    private var tabContentSection: some View {
        TabView(selection: $store.currentTap.sending(\.pageChanged)) {
            ...
            .tag(MyProfileFeature.State.Tab.feed.rawValue)
            ...
            VStack(spacing: 0) {
             ...
             .tag(MyProfileFeature.State.Tab.session.rawValue)
        }
...
#Preview {
    MyProfileView(
        store: Store(initialState: MyProfileFeature.State()) {
            MyProfileFeature()
        }
    )
}

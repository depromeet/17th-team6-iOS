...
struct FeedDetailFeature {
    ...
    struct State: Equatable {
        ...
        @Presents var editMyFeedDetail: EditFeedDetailFeature.State?
    }
    ...
    enum Action: Equatable {
        ...
        /// 수정 화면 액션
        case editMyFeedDetail(PresentationAction<EditFeedDetailFeature.Action>)
    ...
    var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            case let .reactionSuccess(result):
                state.feed.reactions = FeedDetailFeature.toggleReaction(
            ...
            case let .addReactionSuccess(result):
                state.feed.reactions = FeedDetailFeature.addOrToggleReaction(
                    in: state.feed.reactions,
            ...
            case .editButtonTapped:
                // 피드 수정 화면으로 이동
                state.editMyFeedDetail = EditFeedDetailFeature.State(feed: state.feed)
                return .none
...
private extension FeedDetailFeature {
    /// 리액션 탭 시 상태를 토글합니다.

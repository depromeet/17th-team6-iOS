...
final class MockRunningRepository: RunningRepositoryProtocol {
    ...
    // 프로파일(기본 속도/케이던스)
    private var speedMps: Double = 8.5 / 3.6           // 8.5 km/h
    ...
    private func startTicker() {
        ...
        tickerTask = Task { [weak self] in
            ...
            while !Task.isCancelled {
                ...
                // 새로운 위치 샘플 생성
                let now = Date()
                let newLoc = CLLocation(latitude: self.currentLat, longitude: self.currentLon)
                
                // 이전 위치가 있으면 CoreLocation으로 거리 누적
                if let prev = self.lastLocation {
                    self.totalDistanceMeters += newLoc.distance(from: prev)
                }
                self.lastLocation = newLoc
                self.lastLocationTs = now
    ...
    private func yieldSnapshot() {
        ...
        let point: RunningPoint? = lastLocation.map {
            RunningPoint(
                ...
                speedMps: Double.random(in: 1...25)
            )
        }
